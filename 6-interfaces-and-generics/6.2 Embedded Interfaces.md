...
From 6.1, there's still some code duplication we can work on getting rid of. We already have `saveData`, but lets now also create `outputData` which will call `saveData` and also display the data. For this, we can create a new interface `Outputable`, and then we can embed `Saver` and have a `String() string` method.
