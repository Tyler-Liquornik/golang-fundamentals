...
If you run go build in a module without any package main, Go will compile the code but won’t produce an executable, since only packages named `main` can be built into runnable programs. If you never anticipate needing to run a package and only to import it elsewhere, it needs a unique name to import it.
